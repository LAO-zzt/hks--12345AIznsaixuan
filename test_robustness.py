...
from modules import (
    ...
    event_profiler, risk_analyzer, action_advisor, feishu_pusher,
)
...
def full_chain(df_raw, **cluster_kwargs):
    """跑 loader→...→建议 的完整链条，返回 (df, events, info)。"""
...
def main():
    ...
    if events:
        check("趋势降级为“无法判断”", all(e["trend"] == "无法判断" for e in events))
        check("风险等级仍可输出", all(e.get("risk_level") for e in events))

    # 3) Embedding 模型缺失自动回退
