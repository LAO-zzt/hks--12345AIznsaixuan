...
classifier→event_profiler→risk_analyzer→action_advisor→exporter，
用于快速确认最小闭环与业务价值层是否跑通。
...
from modules import (
    ...
    event_profiler, risk_analyzer, action_advisor, exporter,
)
...
def main():
    ...
    for e in events:
        print("  %s %s·%s | 频次%d | 趋势%s | %s(%s) | %s" % (
            e["event_id"], e["event_subject"], e["event_type"],
            e["frequency"], e["trend"],
            e.get("risk_level"), e.get("priority_score"),
            e.get("action_department")))
        print("       原因：%s" % e.get("risk_reason"))
